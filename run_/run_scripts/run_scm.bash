...
NAMELISTS="clubb.in"
FLAGS_FILE="../input/configurable_model_flags.in"
SILHS_PARAMS_FILE="../input/silhs_parameters.in"
CUSTOM_OUTPUT_DIR=""
...
then
        ...
        if [ -z "$outputgrep" ]
        then
          stats_file="../input/standard_stats.in"
        else
...
then
	...
	case $run_case in
		...
		* )
			case $run_case in
				...
				* )
					cat $parameter_file $SILHS_PARAMS_FILE > $NAMELISTS
                    cat $SILHS_PARAMS_FILE >> $NAMELISTS
					cat $FLAGS_FILE >> $NAMELISTS
	...
	cat $parameter_file $SILHS_PARAMS_FILE $FLAGS_FILE > $NAMELISTS
